...
definition desargues_config_3D :: "[Points, Points, Points, Points, Points, Points, Points, Points, Points, Points] => bool" 
...
proof-
  ...
  have "rk {A, B, C, \<alpha>} + rk {B, C} \<le> rk {A, B, C} + rk {B, C, \<alpha>}" using matroid_ax_3_alt
    by (metis Un_insert_right add_One_commute add_mono assms(1) assms(2) matroid_ax_2_alt nat_1_add_1 numeral_plus_one rk_singleton semiring_norm(3) sup_bot.right_neutral)
  then have f2:"rk {A, B, C, \<alpha>} \<le> 3"
    by (metis Un_insert_right add_One_commute assms(2) matroid_ax_2_alt numeral_plus_one semiring_norm(3) sup_bot.right_neutral)
...
proof-
  show "rk {A, B, C, \<alpha>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4 by blast
  show "rk {A', B', C', \<alpha>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4 by blast
qed
...
proof-
  ...
  have "rk {A, B, C, \<beta>} + rk {A, C} \<le> rk {A, B, C} + rk {A, C, \<beta>}" using matroid_ax_3_alt
    by (smt One_nat_def add_Suc_right add_leD1 add_le_mono assms(1) assms(2) insert_commute insert_is_Un matroid_ax_3 numeral_2_eq_2 numeral_3_eq_3 one_add_one rk_singleton)
  then have f2:"rk {A, B, C, \<beta>} \<le> 3"
    by (smt One_nat_def Suc_le_mono add_le_cancel_right assms(1) assms(2) f1 insert_absorb2 insert_commute nat.inject numeral_2_eq_2 numeral_3_eq_3 rk_couple rk_singleton_bis)
...
proof-
  show "rk {A, B, C, \<beta>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4_bis by auto
  show "rk {A', B', C', \<beta>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4_bis by auto
qed
...
proof-
  ...
  then have f2:"rk {A, B, C, \<gamma>} \<le> 3"
    by (smt One_nat_def Un_assoc Un_commute add.commute add_Suc_right assms(2) insert_is_Un matroid_ax_2_alt numeral_2_eq_2 numeral_3_eq_3 one_add_one)
...
proof-
  show "rk {A, B, C, \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4_ter by auto
  show "rk {A', B', C', \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_4_ter by auto
qed
...
proof-
  have f1:"rk {A, B, C, \<alpha>} = 3" using coplanar_4
    by (smt One_nat_def Un_assoc Un_commute add.commute add_Suc_right assms(1) assms(2) insert_is_Un le_antisym matroid_ax_2_alt numeral_2_eq_2 numeral_3_eq_3 one_add_one)
  have f2:"rk {A, B, C, \<beta>} = 3" using coplanar_4_bis
    by (smt One_nat_def Un_assoc Un_commute add.commute add_Suc_right assms(1) assms(3) insert_is_Un le_antisym matroid_ax_2_alt numeral_2_eq_2 numeral_3_eq_3 one_add_one)
...
proof-
  show "rk {A, B, C, \<alpha>, \<beta>} = 3"
    using assms desargues_config_3D_def coplanar_5 by auto
  show "rk {A', B', C', \<alpha>, \<beta>} = 3"
    using assms desargues_config_3D_def coplanar_5 by auto
qed
...
proof-
  show "rk {A, B, C, \<alpha>, \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_5_bis by auto
  show "rk {A', B', C', \<alpha>, \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_5_bis by auto
qed
...
proof-
  have f1:"rk {A, B, C, \<alpha>, \<gamma>} = 3" 
    using coplanar_5_bis assms(1) assms(2) assms(3) by auto
  have f2:"rk {A, B, C, \<alpha>, \<beta>} = 3"
    using coplanar_5 assms(1) assms(2) assms(4) by auto
  have f3:"rk {A, B, C, \<alpha>} = 3" 
    using coplanar_4 assms(1) assms(2) by auto
...
proof-
  show "rk {A, B, C, \<alpha>, \<beta>, \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_6 by auto
  show "rk {A', B', C', \<alpha>, \<beta>, \<gamma>} = 3"
    using assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] coplanar_6 by auto
qed
...
proof-
  ...
  thus "4 \<le> rk {A, B, C, A', B', C', \<alpha>, \<beta>, \<gamma>}" 
    using matroid_ax_2 assms desargues_config_3D_def[of A B C A' B' C' P \<alpha> \<beta> \<gamma>] by linarith
qed
...
proof-
  ...
  then have "rk {\<alpha>, \<beta>, \<gamma>} \<le> rk {A, B, C, \<alpha>, \<beta>, \<gamma>} + rk {A', B', C', \<alpha>, \<beta>, \<gamma>} - rk {A, B, C, A', B', C', \<alpha>, \<beta>, \<gamma>}"
    by linarith
  thus "rk {\<alpha>, \<beta>, \<gamma>} \<le> 2" using assms desargues_config_3D_coplanar_6 desargues_config_3D_non_coplanar
    by fastforce
qed
