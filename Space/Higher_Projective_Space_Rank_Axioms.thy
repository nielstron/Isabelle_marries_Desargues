...
matroid_ax_1a: "\<forall>X. rk X \<ge> 0" (* Useless if rk is defined with values in \<nat>, not \<int> *) and
...
rk_ax_3_pts: "\<forall> A B. \<exists>C. rk {A,B,C} = 2 \<and> rk {B,C} = 2 \<and> rk {A,C} = 2" and
